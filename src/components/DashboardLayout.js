...
export default function DashboardLayout({ children, title }) {
  ...
  const styles = {
    ...
    logoIcon: {
      ...
      fontSize: 18,
      flexShrink: 0,
      boxShadow: '0 0 20px rgba(0,212,245,0.3)',
    },
    logoText: {
      ...
      <aside style={styles.sidebar}>
        <div style={styles.logoArea}>
          ...
          <div style={styles.logoText}>
            <div style={{ fontFamily: 'var(--font-display)', color: 'var(--nova-cyan)', fontSize: '1rem', lineHeight: 1 }}>NovaRad</div>
            <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginTop: 2, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Center</div>
          </div>
        </div>
        ...
        <nav style={styles.nav}>
              ...
              return (
                <Link
                  ...
                  style={{
                    ...
                    borderRadius: 10,
                    textDecoration: 'none',
                    color: active ? 'var(--nova-cyan)' : 'var(--text-secondary)',
                ...
                >
                  <span style={{ fontSize: '1.1rem', flexShrink: 0, width: 20, textAlign: 'center' }}>{link.icon}</span>
                  {!collapsed && <span>{link.label}</span>}
                  {active && !collapsed && (
                    <div style={{ marginLeft: 'auto', width: 6, height: 6, borderRadius: '50%', background: 'var(--nova-cyan)', flexShrink: 0 }} />
                  )}
                </Link>
              );
            })}
          ...
          <div style={{ ...styles.navSection, marginTop: 8 }}>
            ...
            <Link to="/" style={{
              ...
              color: 'var(--text-muted)', fontSize: '0.875rem', whiteSpace: 'nowrap', overflow: 'hidden',
            }}>
              <span style={{ fontSize: '1.1rem', flexShrink: 0, width: 20, textAlign: 'center' }}>⌂</span>
        ...
        <div style={styles.bottomArea}>
          {!collapsed && (
            <div style={{ padding: '10px 12px', borderRadius: 10, background: 'rgba(0,212,245,0.05)', border: '1px solid var(--border)', marginBottom: 12 }}>
              <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Signed in as</div>
              <div style={{ fontSize: '0.875rem', color: 'var(--text-primary)', fontWeight: 600, marginTop: 2 }}>{user?.username}</div>
              <div style={{ fontSize: '0.7rem', color: 'var(--nova-cyan)', textTransform: 'uppercase', letterSpacing: '0.08em', marginTop: 2 }}>{user?.role}</div>
            </div>
          )}
          <button
      ...
      <div style={styles.main}>
        <div style={styles.topbar}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <h1 style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--text-primary)', fontFamily: 'var(--font-display)' }}>
              {title}
            </h1>
          </div>
          <div style={styles.userChip}>
            <div style={styles.avatar}>{(user?.username || 'U')[0].toUpperCase()}</div>
            <div>
              <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-primary)' }}>{user?.username}</div>
              <div style={{ fontSize: '0.65rem', color: 'var(--nova-cyan)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{user?.role}</div>
            </div>
          </div>
        </div>
