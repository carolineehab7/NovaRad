  ...
  return (
    <DashboardLayout title="System Overview">
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 20, marginBottom: 32 }}>
        <StatCard value={data?.total_patients || 0} label="Total Patients" icon="◎" delay={0} />
        <StatCard value={data?.total_staff || 0} label="Staff Members" icon="◑" color="#a78bfa" delay={1} />
        <StatCard value={data?.pending_appts || 0} label="Scheduled Appointments" icon="◷" color="#fbbf24" delay={2} />
        <StatCard value={data?.unpaid_invoices || 0} label="Unpaid Invoices" icon="◈" color="#ff4444" delay={3} />
        <StatCard value={`${(data?.total_revenue || 0).toLocaleString()} EGP`} label="Revenue Collected" icon="✦" color="#00e676" delay={4} />
      </div>
